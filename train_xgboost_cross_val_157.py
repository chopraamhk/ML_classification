...
xgb_model = XGBClassifier(scale_pos_weight=scale_pos_weight, eval_metric="logloss", random_state=42, n_jobs=2)
...
scores = cross_val_score(xgb_model, X, y, cv=cv, scoring="accuracy", n_jobs=2)
...
grid_search = GridSearchCV(xgb_model, param_grid, cv=cv, scoring="accuracy", n_jobs=2, verbose=1)
...
grid_search = GridSearchCV(xgb_model, param_grid, cv=inner_cv, scoring="accuracy", n_jobs=2)

nested_scores = cross_val_score(grid_search, X, y, cv=outer_cv, n_jobs=2)
